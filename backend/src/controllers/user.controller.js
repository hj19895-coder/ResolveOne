...
export const updateUser = async (req, res) => {
  try {
    ...
    // Prevent editing a system user's role/identity via this endpoint
    if (existingUser.role.isSystem && roleId && roleId !== existingUser.roleId) {
      return res.status(403).json({ message: "Cannot change role of a system user" });
    }

    // If changing role, verify the new role exists and isn't a system role
    if (roleId && roleId !== existingUser.roleId) {
      const role = await prisma.role.findUnique({ where: { id: roleId } });
      if (!role) return res.status(400).json({ message: "Selected role does not exist" });
      if (role.isSystem) return res.status(403).json({ message: "Cannot assign a system role" });
    }
