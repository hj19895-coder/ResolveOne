// src/components/reports/TicketHealthCard.jsx
...
function getCellStyle(value, rule) {
  ...
  if (rule === "p1"     && value > 0)  return "text-red-600 font-semibold";
  if (rule === "reopen" && value > 10) return "text-amber-600 font-semibold";
  return "";
}

function getCellBg(value, rule) {
  if (rule === "p1" && value > 0) return "bg-red-50";
  return "";
}
...
function SkeletonRow({ index }) {
  return (
    <tr className={index % 2 === 0 ? "bg-white" : "bg-gray-50/50"}>
      <td className="px-3 py-2.5"><div className="h-3 bg-gray-200 rounded-full animate-pulse w-40" /></td>
      <td className="px-3 py-2.5"><div className="h-3 bg-gray-200 rounded-full animate-pulse w-10 ml-auto" /></td>
      <td className="px-3 py-2.5"><div className="h-3 bg-gray-200 rounded-full animate-pulse w-10 ml-auto" /></td>
    </tr>
  );
}

export default function TicketHealthCard({ onDataLoad }) {
    ...
    <div className="flex items-start gap-5" style={{ color: dark ? "#e5edf7" : "#111827" }}>
      ...
      >
        <div
          ...
          style={{ borderBottom: dark ? "1px solid rgba(148,163,184,0.14)" : "1px solid rgb(243 244 246)" }}
        >
          <div>
          ...
          <table className="border-collapse" style={{ width: W }}>
            <thead>
              <tr style={{ background: dark ? "rgba(255,255,255,0.03)" : "rgb(249 250 251)", borderBottom: dark ? "1px solid rgba(148,163,184,0.14)" : "1px solid rgb(229 231 235)" }}>
                <th className="px-3 py-2.5 text-left text-[10.5px] font-semibold uppercase tracking-wider" style={{ width: COL.cat, color: dark ? "#94a3b8" : "#9ca3af" }}>
                  Category
                </th>
                <th className="px-3 py-2.5 text-right text-[10.5px] font-semibold uppercase tracking-wider whitespace-nowrap" style={{ width: COL.today, color: dark ? "#94a3b8" : "#9ca3af" }}>
                  Today
                </th>
                <th className="px-3 py-2.5 text-right text-[10.5px] font-semibold uppercase tracking-wider whitespace-nowrap" style={{ width: COL.weekly, color: dark ? "#94a3b8" : "#9ca3af" }}>
                  Last 7 Days
                </th>
              </tr>
            </thead>
            <tbody>
              {loading
                ? Array.from({ length: TICKET_HEALTH_ROWS.length }).map((_, i) => <SkeletonRow key={i} index={i} />)
                    ...
                    return (
                      <tr
                        ...
                        style={{
                          ...
                          borderBottom: row.dividerAfter
                            ? (dark ? "2px solid rgba(148,163,184,0.18)" : "2px solid rgb(229 231 235)")
                            : (dark ? "1px solid rgba(148,163,184,0.12)" : "1px solid rgb(243 244 246)"),
                        }}
                      >
                        <td
                        ...
                        <td className={`px-3 py-2.5 text-[12.5px] text-right tabular-nums ${getCellBg(tv, row.highlightRule)} ${getCellStyle(tv, row.highlightRule) || ""}`} style={{ color: dark ? "#e5edf7" : "#1f2937" }}>
                          {formatValue(tv, row.formatter)}
                        </td>
                        <td className={`px-3 py-2.5 text-[12.5px] text-right tabular-nums ${getCellBg(wv, row.highlightRule)} ${getCellStyle(wv, row.highlightRule) || ""}`} style={{ color: dark ? "#e5edf7" : "#1f2937" }}>
                          {formatValue(wv, row.formatter)}
                        </td>
                      </tr>
                    );
                  })}
